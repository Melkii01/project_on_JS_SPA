...
export class IncomeExpense {
    constructor() {
        this.todayDate = new Date().toLocaleDateString().split('.');
        this.todayDateRevers = this.todayDate[2] + '-' + this.todayDate[1] + '-' + this.todayDate[0];

        this.getIncomeExpenseData('all');
    }
    ...
    async getIncomeExpenseData(period) {
        try {
            const result = await CustomHttp.request(config.host +
    ...
    // Достаем операцию
    async getIncomeExpenseItem(itemId) {
        try {
            const result = await CustomHttp.request(config.host + '/operations/' + itemId);
    ...
    showIncomeExpense() {
        ...
        this.resultData.forEach((item) => {
            ...
            if (item.type === 'income') {
                const getItem = this.getBudgetData(item.type)
                    .then((operation) => {
                        return operation.find((category) => {
            ...
            // Опция редактирования и удаления
            ...
            linkEdit.insertAdjacentHTML('beforeend', `<svg class="table-svg" width="14" height="15" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            linkDelete.insertAdjacentHTML('beforeend', `<svg class="table-svg" xmlns="http://www.w3.org/2000/svg" width="16" height="15" viewBox="0 0 16 16" fill="none">
            ...
            this.getIncomeExpenseItem(item.id)
